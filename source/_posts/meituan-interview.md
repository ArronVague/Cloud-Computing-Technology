...
- Ceph提供了一个完全兼容S3和Swift的RESTful API，可以很容易地与Web应用程序集成，可以通过HTTP或HTTPS协议方便地访问这些文件。
- **对象存储**：是存储大量非结构化数据（如图片），且多读少写的理想选择，因为它可以轻松地处理大量的小文件。
- **高可用性和数据冗余**：Ceph通过复制或纠删码（Erasure Coding）提供数据冗余，这可以防止单点故障导致的数据丢失。这对于存储重要的图片文件来说是非常重要的。
- **可扩展性**：Ceph是一个高度可扩展的存储系统，可以无缝地扩展到数百个或数千个节点，这使得它非常适合存储大量的图片文件。
- **成本效益**：与商业存储解决方案相比，Ceph作为一个开源项目，可以在 commodity hardware上运行，这可以大大降低存储成本。
- **集群管理和监控**：Ceph提供了一套完整的集群管理和监控工具，使得运维人员可以方便地管理和监控存储集群的状态。
- **与现有架构的集成**：如果你的应用已经在使用Ceph或者你的应用正在使用支持Ceph的云平台，那么使用Ceph存储图片可以更好地与你的现有架构集成。

存储数据主要有三种方式：块存储、文件存储和对象存储

- 块存储

  以数据块为单位进行存储，每个数据块都有直接的编号，没有文件系统，因此块存储无法在不同文件系统之间共享。
...
- 文件存储
  ...
  采用树状目录结构来管理和组织方式，也就是我们常见的文件和文件夹结构，这种存储方式提供了文件系统层次结构，用户和应用程序可以通过文件路径进行数据访问
...
- 对象存储
  ...
  是一种基于对象的存储结构，它的结构类似于Key-Value结构，Key对应的是全局唯一标识符UID，通过UID唯一标识每个对象；Value对应文件数据和相关元数据。对象存储能够以RESTful API形式访问资源。
